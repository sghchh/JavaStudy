...
在《Java编程思想》中作者有一句话：**泛型发生在边界处，即对象进入和离开方法的地方。**  
...
例子：  
	...
	test1.add("hello");//ok
	//test1.add(1);//error
	String a=test1.get(0);

当对象进入方法(如上面的set方法)的时候回进行泛型检查，检查通过后编译成的字节码中就没有了任何泛型的类型信息了；而当对象出去方法的时候，自动插入类型转换(如上面的get直接就是String类型)。**不光是方法进出为边界，对泛型类型变量的赋值和取出也是一样**  
...
例如：  
	...
	a.T="hello";//ok
	a.T=1;//error
	String h=a.T;//OK,自动插入类型转换  

### 2. 通配符与边界  
泛型中的通配符可以实现类似于“泛型继承”：  

	ArrayList<? extends Object> list1;
	ArrayList<String> list2=new ArrayList();
	list1=list2;    //由于这里使用了  ？extends  编译是可以通过的，但是如果不是这样的话，编译失败  

《Java核心技术》中关于这种现象有一个很通俗的解释,我想我们可以用来理解：  

> ArrayList<? extends Object> list1这句话的意思是，泛型类型为**Object的一个子类型就可以，而在编译期进行泛型类型检查的时候，发现list2的泛型满足这个要求(String extends Object)所以编译通过**  

使用上面的形式虽然可以实现一种“泛型继承”的假象，但是，这种形式是**只读的，这意味着，泛型参数只能作为方法中的返回值，而不能作为参数**：  

	ArrayList<? extends Object> list;
	ArrayList<String> list2=new ArrayList<String>();
	list2.add("hello");
	list=list2;
	String t=list.get(0);   //OK
	list.add("world");     //error  

这种情况我们还是按照《Java核心技术》卷1中的解释那样：**首先解释为什么读取泛型变量是可以的：因为我们在定义的时候是Object的任何一个子类型A，能够安全的取出那是因为我们都知道A一定是边界类型(泛型擦除时擦除为边界的类型，这里是Object)；但是由于边界的子类型可能有多个，由于为list赋值时？已经取为了一个特定的子类型A，往里面写入一个变量的时候，这个变量可能是边界的子类型，但是不能保证就是A这种子类型，他有可能是B这种子类型(注意? extends XXX 表示边界的任何一个子类型就可以，但是一旦选定，就是这种特定的类型了)，所以就干脆为只读的**   

对应的还有一个**? super XXX**形式，大部分都和上面的一样，但是相反的这种形式是**只写的，这意味着，泛型参数只能作为参数而不能作为方法的返回值**；还是一样类似的解释，**为什么可以写入、作为方法的参数；这是因为我们最起码可以把它作为边界而存入，由于边界是具体类型的子类型，这意味着，边界的成员包含具体类型的所有，所以无论你想要在方法体中做任何操作，都没有问题；但是作为返回值，由于不知道具体的类型是什么(擦除为了边界类型)**
